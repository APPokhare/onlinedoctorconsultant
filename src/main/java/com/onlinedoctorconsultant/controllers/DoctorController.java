...
import com.onlinedoctorconsultant.services.DoctorService;
...
public class DoctorController implements IBaseController<Doctor, Long>, IRestConstants {
	...
	@GetMapping(SEPARATOR + OPEN_BRACKET + USERNAME + CLOSE_BRACKET + SEPARATOR + OPEN_BRACKET + SPECIALIZATION + CLOSE_BRACKET)
	public List<DoctorDTO> getAllNearbyDoctors(@PathVariable(USERNAME) String username, @PathVariable(SPECIALIZATION) String specialization) {
		return doctorService.getAllNearbyDoctors(specialization, username);
	}
